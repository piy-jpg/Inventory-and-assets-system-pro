...
const Payments = () => {
  ...
  const { data: paymentsData, isLoading, error } = useQuery(
    ['payments', { search, status: filterStatus, paymentMethod: filterMethod, startDate: dateRange.startDate, endDate: dateRange.endDate }],
    () => paymentsAPI.getAll({ search, status: filterStatus, paymentMethod: filterMethod, startDate: dateRange.startDate, endDate: dateRange.endDate }),
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage and track all payment transactions</p>
        </div>
        <button
          onClick={() => navigate('/payment-accounts', { state: { openTab: 'transactions', openTransactionForm: true, transactionType: 'deposit' } })}
                ...
                <tr key={payment._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{payment.customer_id}</div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${getPaymentMethodColor(payment.payment_method)}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {payment.payment_type.replace('_', ' ').charAt(0).toUpperCase() + 
                       payment.payment_type.replace('_', ' ').slice(1)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
