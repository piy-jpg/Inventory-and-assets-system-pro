...
const Quotations = () => {
  ...
  const { data: quotationsData, isLoading, error } = useQuery(
    ['quotations', { search, status: filterStatus, startDate: dateRange.startDate, endDate: dateRange.endDate }],
    () => quotationsAPI.getAll({ search, status: filterStatus, startDate: dateRange.startDate, endDate: dateRange.endDate }),
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage and track all customer quotations and drafts</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {formatCurrency(quotations.reduce((sum, q) => sum + (q.total_amount || 0), 0))}
              </p>
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {quotations.map((quotation) => {
                const validityStatus = getValidityStatus(quotation.valid_until);
                return (
                  <tr key={quotation._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{quotation.customer_id}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-900">{formatDate(quotation.valid_until)}</span>
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${validityStatus.color}`}>
                          {validityStatus.text}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <button className="text-green-600 hover:text-green-900" title="Convert to Invoice">
